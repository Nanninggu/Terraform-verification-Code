...
# private subnet 1과 라우트 테이블 연결 (추가)
resource "aws_route_table_association" "private_1" {
  subnet_id = aws_subnet.private1.id  # 프라이빗 서브넷 1의 ID 설정
  route_table_id = var.route_table_id1  # 라우트 테이블 ID 설정
}

# private subnet 2와 라우트 테이블 연결 (추가)
resource "aws_route_table_association" "private_2" {
  subnet_id = aws_subnet.private2.id  # 프라이빗 서브넷 2의 ID 설정
  route_table_id = var.route_table_id1  # 라우트 테이블 ID 설정
}